...
$srcBase = "d:\Projects\Golyath\exercises"
$dstBase = "d:\Projects\Golyath\src\Golyath\Resources\Raw\exercises"

# Create base destination dir
if (-not (Test-Path $dstBase)) { New-Item -ItemType Directory -Path $dstBase | Out-Null }

foreach ($ex in $selected) {
  $dstDir = Join-Path $dstBase $ex
  if (-not (Test-Path $dstDir)) { New-Item -ItemType Directory -Path $dstDir | Out-Null }
  
  Copy-Item -Path "$srcBase\$ex\0.jpg" -Destination "$dstDir\0.jpg" -Force
  Copy-Item -Path "$srcBase\$ex\1.jpg" -Destination "$dstDir\1.jpg" -Force
}

Write-Host "Copied $($selected.Count) exercise image folders to Raw assets."
